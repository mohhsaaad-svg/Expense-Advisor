  ...
  const createChallenge = useCreateChallenge({
    mutation: {
      onSuccess: () => {
        ...
        toast({ title: "Challenge started", description: "You've got this!" });
      }
    }
  });
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 pb-4 border-b border-border/50">
        <div>
          <h1 className="text-4xl font-serif font-bold tracking-tight text-foreground mb-2">Targets</h1>
          <p className="text-lg text-muted-foreground font-light">Savings goals and no-spend challenges.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-serif font-bold flex items-center gap-2"><Target className="w-6 h-6 text-primary" /> Savings Goals</h2>
            <Dialog open={isGoalDialogOpen} onOpenChange={setIsGoalDialogOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="rounded-full"><Plus className="w-4 h-4 mr-1" /> New Goal</Button>
              </DialogTrigger>
          ...
          {!loadingGoals && goals?.length === 0 && (
             <div className="p-8 rounded-2xl bg-secondary/30 border border-dashed border-border flex flex-col items-center justify-center text-center">
               ...
               <h3 className="font-serif text-xl font-bold mb-2">No goals yet</h3>
               <p className="text-muted-foreground max-w-[250px] mb-6 font-light">Set a target to keep your spending intentional.</p>
               <Button onClick={() => setIsGoalDialogOpen(true)}>Create your first goal</Button>
             </div>
          )}
                    ...
                    <div>
                      ...
                      {goal.deadline && (
                        <CardDescription className="text-xs mt-1 flex items-center gap-1">
                          <Calendar className="w-3 h-3" /> By {format(new Date(goal.deadline), "MMM d, yyyy")}
                        </CardDescription>
                      )}
                    </div>
                  ...
                  <CardContent>
                    <div className="flex items-end justify-between mb-2">
                      <div className="font-sans">
                        ...
                        <span className="text-muted-foreground text-sm ml-1">/ {formatCurrency(goal.targetAmount)}</span>
                      </div>
                    ...
                    <div className="flex gap-2">
                      <Dialog open={contributeGoalId === goal.id} onOpenChange={(open) => {
                        ...
                        <DialogTrigger asChild>
                          <Button variant="outline" size="sm" className="w-full text-xs font-medium"><ArrowUpRight className="w-3 h-3 mr-1" /> Add</Button>
                        </DialogTrigger>
                        <DialogContent className="sm:max-w-sm">
                          ...
                          <Form {...contributeForm}>
                            <form onSubmit={contributeForm.handleSubmit(onContributeSubmit)} className="space-y-4">
                              ...
                              <DialogFooter>
                                <Button type="submit" disabled={contributeGoal.isPending}>Save</Button>
                              </DialogFooter>
                            </form>
                          </Form>
                        </DialogContent>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-serif font-bold flex items-center gap-2"><Shield className="w-6 h-6 text-primary" /> Challenges</h2>
            <Dialog open={isChallengeDialogOpen} onOpenChange={setIsChallengeDialogOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="rounded-full"><Plus className="w-4 h-4 mr-1" /> New Challenge</Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  <DialogTitle>Start a No-Spend Challenge</DialogTitle>
                </DialogHeader>
                <Form {...challengeForm}>
                  <form onSubmit={challengeForm.handleSubmit(onChallengeSubmit)} className="space-y-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="No takeout November" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Category to block</FormLabel>
                          <Select onValueChange={field.onChange} defaultValue={field.value || "all"}>
                            ...
                            <SelectContent>
                              <SelectItem value="all">Block ALL Spending</SelectItem>
                              {CATEGORIES.map((c) => (
                    ...
                    />
                    <DialogFooter>
                      <Button type="submit" disabled={createChallenge.isPending}>Start Challenge</Button>
                    </DialogFooter>
                  </form>
                </Form>
              </DialogContent>
          ...
          {!loadingChallenges && challenges?.length === 0 && (
             <div className="p-8 rounded-2xl bg-secondary/30 border border-dashed border-border flex flex-col items-center justify-center text-center">
               ...
               <h3 className="font-serif text-xl font-bold mb-2">No active challenges</h3>
               <p className="text-muted-foreground max-w-[250px] mb-6 font-light">Test your discipline with a no-spend window.</p>
               <Button onClick={() => setIsChallengeDialogOpen(true)} variant="secondary">Start a challenge</Button>
             </div>
          )}
                  ...
                  <CardHeader className="pb-2 flex flex-row items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {isFailed && <span className="bg-destructive/20 text-destructive text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">Failed</span>}
                        {isCompleted && <span className="bg-success/20 text-success text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">Completed</span>}
                        {isActive && <span className="bg-primary/20 text-primary text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">Active</span>}
                      </div>
                      <CardDescription className="text-xs">
                        {challenge.category ? `No spending on ${challenge.category}` : "No spending at all"} • {challenge.durationDays} days
                      </CardDescription>
                    </div>
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <CardContent>
                    <div className="flex items-end justify-between mb-2">
                      <div className="font-sans">
                        ...
                        <span className="text-muted-foreground text-sm ml-1">/ {challenge.durationDays}</span>
                      </div>
                    ...
                    {isFailed && (
                      <div className="flex items-center gap-2 text-sm text-destructive font-medium bg-destructive/10 p-3 rounded-lg">
                        <X className="w-4 h-4" />
                        Broken by {challenge.violations} expense{challenge.violations > 1 ? 's' : ''}. Try again!
                      </div>
                    )}
                    {isCompleted && (
